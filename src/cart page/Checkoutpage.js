...
export default function Checkoutpage() {
  ...
  const MakePayment = async (token) => {
    try {
      const total = cart.cartTotalaAmount + 150;
      const product = name;
      const two = {
        total,
        product,
      ...
      const value = await axios.post("http://localhost:5000/payment", {
        tokenValue,
      });
            ...
            <div className="relative py-10">
              <div className="flex absolute right-0 top-5">
                {current < steps.length - 1 && (
                  <Button
                    onClick={() => next()}
                    className="bg-[#b4b4b4] text-white flex font-bold hover:!text-white hover:bg-blue-600"
                  >
            ...
            <div className="w-[400px] shadow-lg mb-5">
              {cart.cartItems?.map((d, index) => {
                return (
                  <div
                    key={index}
                    className="flex border-[0.01px] border-b-[#ccc9c9] "
                  >
                    ...
                    <div className="mt-10 text-[#afafaf] font-semibold">
                      ...
                      <h3 className=" flex">
                        Price: <BsCurrencyRupee className="mt-[5px] ml-2" />
                        {d.off ? d.off : d.price}.00
                      </h3>
                    </div>
                  </div>
                );
